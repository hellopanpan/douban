        ...
        <div class="content" v-for="item in data" v-bind:key="item.uri">
          ...
          >
            <img
              ...
              "
            />
            <img class="pic" :src="item.picIndex" />
          </div>
...
export default {
  ...
  methods: {
        ...
        .then(function (res) {
          ...
          for (var i = 0; i < vm.data.length; i++) {
            for (var j = 0; j < vm.data[i].items.length; j++) {
              ...
              vm.data[i].items[j].uri = str.replace(
                /douban\:\/\/douban\.com/,
    ...
    goUri(url) {
      ...
      Axios.post('/cross/douban/video/con', { url: url })
        .then(function (res) {
          vm.dataV = res.data
